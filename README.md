...
**Reproducing the committed golden corpus (#94):** `datasets/goldens/qa_dataset.json` and `retrieval_dataset.json` were built from `datasets/goldens/rag_platform_corpus.md` — committed there rather than under `data/raw/`, since `data/raw/` is gitignored and would silently drop it. Run `make ingest-eval-corpus` to index it; without it, `check_live_retrieval_regression()` has no way to find the golden dataset's `relevant_chunk_ids` and reports `Recall@5 = 0.0` instead of skipping or passing.

**Isolated eval collection/index (#96):** `make ingest-eval-corpus` indexes the golden corpus into a dedicated Qdrant collection (`rag_documents_eval`, `EVAL_COLLECTION_NAME`) and a dedicated disk-backed BM25 index (`data/processed/bm25_eval`, `BM25_EVAL_DISK_PATH`) — both distinct from whatever `qdrant.collection` / `retrieval.bm25.disk_path` a developer's default environment is configured to. `check_live_retrieval_regression()` always targets these dedicated names, never ambient settings. This means any ad-hoc `make ingest SOURCE=...` of unrelated content (e.g. a PDF pulled in for manual testing) can never dilute or contaminate the golden corpus's retrieval signal — the collision that originally caused #94's `Recall@5 = 0.0` false alarm.
