...
client and call `close()` on the previous `httpx` session. Call
`clear_ocr_provider_cache()` after settings reloads in tests (also done
automatically by `temporary_config` / `reload_settings_module`).
